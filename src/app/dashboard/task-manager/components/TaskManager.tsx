...
const TaskManager: React.FC = () => {
    ...
    useEffect(() => {
        if (userData) {
            ...
            setDailyTasks(fetchedTasks.filter(task => task.title.toLowerCase().includes('daily') && !task.complete));
    ...
    const addTask = async (newTask: Task) => {
        try {
            if (userData?.uid) {
                const taskRef = await addDoc(collection(db, `users/${userData.uid}/tasks`), newTask);

                // Update local state based on task type
                if (!newTask.complete) {
                    setActiveTasks(prev => [...prev, { ...newTask, id: taskRef.id }]);
                } else {
                    setCompletedTasks(prev => [...prev, { ...newTask, id: taskRef.id }]);
                }
    ...
    const setTaskAsCompleted = async (taskId: string) => {
        try {
            if (userData?.uid) {
                ...
                // Update local state based on task type
                setActiveTasks(prev => prev.map(task => task.id === taskId ? { ...task, complete: true } : task));
                setDailyTasks(prev => prev.map(task => task.id === taskId ? { ...task, complete: true } : task));
                setCompletedTasks(prev => [...prev, { ...activeTasks.find(task => task.id === taskId) as Task, complete: true }]);
    ...
    return (
        <div className="p-8 px-0 min-h-screen">
            ...
            <div className="w-[90%] mx-auto sm:w-full">
                <Tabs className='w-full mx-auto sm:w-[90%]'>
                    <Tab key={"Active Tasks"} title={<span className='dark:text-white flex justify-center text-sm'>Active Tasks <FaThumbtack className='mt-1 ml-2' /></span>}>
                        ...
                        <TaskTable tableType='active' tasks={activeTasks} title="Your Tasks" onTaskRemove={removeTask} onUpdateTasks={setActiveTasks} onCompleteTask={setTaskAsCompleted} />

                        <TaskTable tableType='daily' tasks={dailyTasks} title="Daily Tasks" onTaskRemove={removeTask} onUpdateTasks={setDailyTasks} onCompleteTask={setTaskAsCompleted} />
                    </Tab>
                    <Tab key={"Completed Tasks"} title={<span className='dark:text-white flex justify-center text-sm'>Completed Tasks <FaCheckSquare className='mt-1 ml-2' /></span>}>
                        <TaskTable tableType='completed' tasks={completedTasks} title="Completed Tasks" onTaskRemove={removeTask} onUpdateTasks={setCompletedTasks} onCompleteTask={setTaskAsCompleted} />
                    </Tab>
                </Tabs>
            </div>
        </div>
